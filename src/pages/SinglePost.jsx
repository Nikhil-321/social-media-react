...
import { ADD_COMMENT } from "../graphql/Mutations/Post";
import { GET_POST_BY_ID } from "../graphql/Queries/Post";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { GET_USER_BY_ID } from "../graphql/Queries/User";
import jwtDecode from "jwt-decode";
import { DELETE_COMMENT, EDIT_COMMENT } from "../graphql/Mutations/Comment";
import ReactTooltip from 'react-tooltip';


const SinglePost = () => {
  const { postId } = useParams();
  const userId = jwtDecode(localStorage.getItem("jwtToken")).userId;

  const {
    data: loginData,
    error: loginUserError,
    loading: loginLoading,
  } = useQuery(GET_USER_BY_ID, { variables: { userId: userId } });

  const [deleteComment] = useMutation(DELETE_COMMENT)

  const editCommentById = (commentId) => {
  ...
  const deleteCommentById = (commentId) => {
    deleteComment({
      ...
      onCompleted(res) {
        console.log("res", res)
        toast.success(res.deleteComment)
      }, 
  ...
  if (loginUserError) return <h2>{loginUserError}</h2>;
  const { username, likes, comments, createdAt, description } =
    data.getPostById;

  return (
    <>
      <>
        <div className="rounded overflow-hidden shadow-lg py-4 block m-auto w-1/2">
          <img
            className="w-1/2"
          ...
          <div className="flex mx-4">
            <button data-tip= {likes.map(e => (e.username))} className=" bg-slate-200 mx-4 px-2 py-1 cursor-pointer rounded-sm">
              <i className={`fa fa-thumbs-up mx-1`} />
              Like {likes.length}
            </button>
            <ReactTooltip />

            <button className=" bg-slate-200 mx-4 px-2 py-1 cursor-pointer rounded-sm">
              {" "}
        ...
        <div
          ...
          className="rounded overflow-hidden shadow-lg py-4 w-1/2 block m-auto"
        >
          {comments.map((e, idx) => (
            <div
              key={idx}
              className=" border px-4 py-4 bg-slate-50 flex justify-between"
            >
              <div className="border-b-2">
                <div className="flex">
                <p className="uppercase font-semibold">{e.username}</p>
                <p className="mx-10 text-xs text-gray-500">{e.createdAt}</p>
                </div>
                <p className="">{e.comment}</p>
              </div>

              {loginData && loginData.getUserById.userName === e.username ? (
                <div>
                  <i onClick={() => deleteCommentById(e.id)} className="fa fa-trash cursor-pointer"></i>
                  <i  onClick={() => editCommentById(e.id)} className="fa fa-edit cursor-pointer mx-2"></i>
                </div>
              ) : (
                ""
              )}
        ...
        <div className="rounded overflow-hidden shadow-lg py-4 w-1/2 block m-auto">
